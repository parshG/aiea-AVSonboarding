...
ENV_ID = "CarRacing-v2"
TOTAL_FRAMES = 300000  # total timesetps, updated from 100000 --> 300000
...
LR = 1e-4  # learning rate, stable at 1e-4
TARGET_UPDATE_FREQ = 1000  # how often (in frames) to copy weights to target net, update from 10000 --> 1000
...
def train():
    ...
    while global_frame < TOTAL_FRAMES:
        ...
        while not done and global_frame < TOTAL_FRAMES:
            ...
            next_state = stack_frames(frames)  # (4,84,84)
        ...
        print(
            f"Episode {episode_idx} | Frames {global_frame} | Reward {episode_reward:.2f} | eps={eps:.3f}"
        )
        ...
        episode_rewards = (
            np.array(
                [
                    event.value
                    for event in writer._get_file_writer()._summary_writer._event_writer._ev_writer._queue
                ]  # ugly, ignore; use explicit tracking if you want
            )
            if False
            else None
        )
